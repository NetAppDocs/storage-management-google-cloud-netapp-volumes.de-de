...
NOTE: Für diese Aufgaben können Sie die Google Cloud console oder die gcloud CLI verwenden. Um die gcloud CLI zu verwenden, müssen Sie link:https://cloud.google.com/sdk/docs/install-sdk["Google Cloud CLI installiert"^]zuerst haben.
...
NOTE: Änderungen an den Google Cloud-Projektberechtigungen können einige Minuten dauern, bis sie wirksam werden.
...
NOTE: Verwenden Sie für Staging-Umgebungen  `credentials-sa@wf-staging-netapp.iam.gserviceaccount.com` anstelle des Produktionsdienstkontos. Ersetzen Sie den  `--member` Parameterwert im obigen Befehl durch  `serviceAccount:credentials-sa@wf-staging-netapp.iam.gserviceaccount.com`.
